/**
 * SDK — Session management for Vyral.
 * Sessions are signed with JWT_SECRET (our own HS256 JWTs).
 * Supabase access tokens are verified in oauth.ts before the session is issued.
 */
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    db.upsertUser({ openId: user.openId, lastSignedIn: new Date() }).catch(
      (err) => console.error("[Auth] Failed to update lastSignedIn:", err)
    );
